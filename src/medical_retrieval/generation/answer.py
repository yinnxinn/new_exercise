...
def build_grounded_answer(query: str, candidates: list[HybridCandidate]) -> str:
    ...
    if len(candidates) > 1:
        related = "；".join(
            f"[{idx}] {item.doc.title}/{item.doc.department}"
            for idx, item in enumerate(candidates[1:4], 2)
        )
        lines.append(f"其他可参考资料：{related}。")

    sources = ", ".join(
        f"[{idx}] {item.doc.id}-{item.doc.title}"
...
def stream_answer(answer: str, chunk_size: int = 18) -> Iterable[AnswerChunk]:
    ...
    for start in range(0, len(answer), chunk_size):
        yield AnswerChunk(answer[start : start + chunk_size])
