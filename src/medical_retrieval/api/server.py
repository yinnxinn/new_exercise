...
def main() -> None:
    """Run the development API server when executed as a Python module."""
    try:
        import uvicorn
    except Exception as exc:  # pragma: no cover - optional dependency
        raise RuntimeError("API server requires `pip install -e .[api]`.") from exc

    host = os.getenv("MEDICAL_RETRIEVAL_HOST", "127.0.0.1")
    port = int(os.getenv("MEDICAL_RETRIEVAL_PORT", "8000"))
